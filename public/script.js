...
function exportToWord() {
	...
	// Try client-side .docx first if docx is available (Pages/Workers safe)
	try {
		if (window.docx && window.docx.Document && window.docx.Packer) {
			...
			const doc = new Document({
				sections: [{
					children: [
						new Paragraph({
							children: [new TextRun({ text: 'DANH SÁCH CÂU HỎI', bold: true, size: 28, font: 'Times New Roman' })],
						...
						...processedQuestions.map(q => [
							new Paragraph({
								children: [
									new TextRun({ text: `Câu ${q.id}: `, bold: true, size: 26, font: 'Times New Roman' }),
									new TextRun({ text: String(q.question || ''), bold: true, size: 26, font: 'Times New Roman' })
								]
							}),
							...q.answers.map((a, idx) => new Paragraph({
								children: [
									new TextRun({ text: `${String.fromCharCode(65 + idx)}. `, size: 24, font: 'Times New Roman' }),
									new TextRun({ text: String(a.answer || ''), size: 24, font: 'Times New Roman' })
								],
								indent: { left: 720 }
							}))
						]).flat()
			...
			Packer.toBlob(doc).then(blob => {
				downloadFile(blob, 'questions.docx');
				closeExportModal();
				showSuccess('Tài liệu đã được xuất thành công!');
			}).catch(() => {
				// Fallback to server
				requestServerWordExport();
			});
			return;
		}
	} catch (_) {
	...
	// Server fallback (.docx on Node, .txt on Pages)
	requestServerWordExport();
}

function requestServerWordExport() {
	fetch('/export-word', {
		method: 'POST',
		headers: {
